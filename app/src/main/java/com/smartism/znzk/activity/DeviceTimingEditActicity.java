...
public class DeviceTimingEditActicity extends ActivityParentActivity implements CompoundButton.OnCheckedChangeListener, View.OnClickListener {
    public static final String TAG = DeviceTimingEditActicity.class.getSimpleName();
    private Context mContext;
    private boolean flag;//定时是否开启或禁用
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        flag = getIntent().getBooleanExtra("status", true);
        initView();
        initData();
    }
    ...
    private void initData() {
        ...
        if (sceneId != -1) {
            if (timerInfo.getControlInfos() != null && timerInfo.getControlInfos().get(0) != null) {
                int status = timerInfo.getControlInfos().get(0).getCommand();
                switchButton.setCheckedImmediatelyNoEvent(status == 1 ? true : false);
            }
            timeBuffer =timerInfo.getCycle();
...
//                    buffer.replace(7, 8, "0");
                    buffer = new StringBuffer("00000000");
                }
                break;
        }
                    ...
                    public void run() {
                        ...
                        if (buffer.toString().equals("00000000")) {
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    cancelInProgress();
                                    Toast.makeText(DeviceTimingEditActicity.this, getString(R.string.qwq_date), Toast.LENGTH_SHORT).show();
//                                    finish();
                                }
                            });
...
//                        }
                        object.put("cc", switchButton.isChecked() ? 1 : 0);
                        ...
                        if (sceneId != -1) {
                            result = HttpRequestUtils
                                    .requestoOkHttpPost(server + "/jdm/s3/dtc/update", o, DeviceTimingEditActicity.this);
                        } else {
                            result = HttpRequestUtils
                                    .requestoOkHttpPost(server + "/jdm/s3/dtc/add", o, DeviceTimingEditActicity.this);
                        }
...
//										+ URLEncoder.encode(
//												SecurityUtil.crypt(pJsonObject.toJSONString())),
//								mContext, mHandler);
        ...
        public void run() {
            ...
            JavaThreadPool.getInstance().excute(new Runnable() {
                ...
                public void run() {
                    ...
                    result = HttpRequestUtils
                            .requestoOkHttpPost(server + "/jdm/s3/scenes/get", pJsonObject, DeviceTimingEditActicity.this);
                    if ("-3".equals(result)) {
                        mHandler.post(new Runnable() {
                            ...
                            public void run() {
                                cancelInProgress();
                                Toast.makeText(DeviceTimingEditActicity.this, getString(R.string.device_not_getdata),
                                        Toast.LENGTH_LONG).show();
                            }
                        });
                        ...
                        mHandler.post(new Runnable() {
                            ...
                            public void run() {
                                cancelInProgress();
                                Log.e(TAG, "loadonescnce:" + result.toString());
                                if (result == null || "".equals(result)) {
                                    cancelInProgress();
                                    Toast.makeText(DeviceTimingEditActicity.this, getString(R.string.device_set_tip_responseerr), Toast.LENGTH_SHORT).show();
                                    return;
                                }
//                                "triggerInfos": [
//                                {
