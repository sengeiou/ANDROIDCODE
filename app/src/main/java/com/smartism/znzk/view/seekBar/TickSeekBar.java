package com.smartism.znzk.view.seekBar;
 ...
 *
 * 导入日期：2020年03月23日，由于滑动条上的字无法被修改，所以将源代码拷贝到项目中。
 * 仅修改getProgressString方法，增加单位
 */
...
public class TickSeekBar extends View {
    ...
    private String mProgressUnit = "";//单位 默认空字符
...
//        initTickXAndTextXLocation();
    ...
    private String getProgressString(float progress) {
        if (mIsFloatProgress) {
            return FormatUtils.fastFormat(progress, mScale) + mProgressUnit;
        }
        return String.valueOf(Math.round(progress)) + mProgressUnit;
    }
     ...
     * @param mProgressUnit
     */
    public void setProgressUnit(String mProgressUnit) {
        this.mProgressUnit = mProgressUnit;
    }
}
