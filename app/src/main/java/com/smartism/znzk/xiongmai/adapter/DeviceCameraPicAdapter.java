package com.smartism.znzk.xiongmai.adapter;
...
import com.smartism.znzk.xiongmai.lib.funsdk.support.FunPath;
import com.smartism.znzk.xiongmai.lib.funsdk.support.FunSupport;
import com.smartism.znzk.xiongmai.lib.funsdk.support.OnFunDeviceFileListener;
...
import com.smartism.znzk.xiongmai.lib.funsdk.support.utils.FileDataUtils;
...
public class DeviceCameraPicAdapter extends BaseAdapter implements OnFunDeviceFileListener {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (info != null) {
            ...
            if (!TextUtils.isEmpty(info.getBeginTimeStr())) {
                ...
                if (info.getFileName().endsWith(".h264")) {
                    ...
                    if ( mPlayingIndex == position ) {
                        holder.event.setTextColor(mContext.getResources().getColor(R.color.zhzj_default));
                        holder.time.setTextColor(mContext.getResources().getColor(R.color.zhzj_default));
                    } else {
                        holder.event.setTextColor(mContext.getResources().getColor(R.color.gray));
                        holder.time.setTextColor(mContext.getResources().getColor(R.color.gray));
                    }
            ...
            if (!TextUtils.isEmpty(type) && type.equals("遥控")) {
                holder.event.setCompoundDrawables(null, null,
                        setTopDrawable(R.drawable.ic_icon_save_svg),
                        null);
            } else {
        ...
        if (info.getFileName().endsWith(".jpg") && mFunDevice.getDevType() == FunDevType.EE_DEV_SPORTCAMERA){
        	...
        	if (info.getCapTempPath() != null) {
				...
				holder.image.setImageResource(R.drawable.ic_icon_video_svg_final);
			}
        }
